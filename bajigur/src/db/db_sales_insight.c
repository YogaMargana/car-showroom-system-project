...
static const char *Q_SUMMARY =
    ...
    "   WHERE pm.StatusPembayaran IN ('Berhasil')) AS TotalMobilTerjual, "
    ...
    "   WHERE pa.StatusPembayaran IN ('Berhasil')) AS TotalAksesorisTerjual, "

    "  (SELECT ISNULL(SUM(CAST(pm.Total AS money)),0) "
    "   FROM dbo.PenjualanMobil pm "
    "   WHERE pm.StatusPembayaran IN ('Berhasil')) AS OmzetMobil, "

    "  (SELECT ISNULL(SUM(CAST(pad.Subtotal AS money)),0) "
    "   FROM dbo.PenjualanAksesoris pa "
    "   JOIN dbo.PenjualanAksesorisDetail pad ON pad.PenjualanAksesorisID = pa.PenjualanAksesorisID "
    "   WHERE pa.StatusPembayaran IN ('Berhasil')) AS OmzetAksesoris, "

    "  ( "
    "    (SELECT COUNT(*) FROM dbo.PenjualanMobil pm WHERE pm.StatusPembayaran IN ('Berhasil')) "
    "    + "
    "    (SELECT COUNT(*) FROM dbo.PenjualanAksesoris pa WHERE pa.StatusPembayaran IN ('Berhasil')) "
    "  ) AS SuccessTx;";
...
static const char *Q_TOP_TX =
    ...
    "  FROM dbo.PenjualanAksesoris pa "
    "  JOIN dbo.Karyawan k ON k.KaryawanID = pa.SalesID "
    "  WHERE pa.StatusPembayaran IN ('Berhasil') "
    "  GROUP BY k.Nama "
    "), "
    "EmpAll AS ( "
