...
static void BuildCustomersView(void)
{
    ...
    for (int i = 0; i < gCustomerCount; i++)
    {
        ...
        {
            ok = false;
            if (UIStringContainsI(gCustomers[i].PelangganID, gTableSearch)) ok = true;
            else if (UIStringContainsI(gCustomers[i].Nama, gTableSearch)) ok = true;
            else if (UIStringContainsI(gCustomers[i].Email, gTableSearch)) ok = true;
            else if (UIStringContainsI(gCustomers[i].NoHp, gTableSearch)) ok = true;
            else if (UIStringContainsI(gCustomers[i].NoKTP, gTableSearch)) ok = true;
            else if (UIStringContainsI(gCustomers[i].Alamat, gTableSearch)) ok = true;
        }

        if (ok) gViewIdx[gViewCount++] = i;
    }
    ...
    if (gSelectedRow >= 0)
    {
        ...
        {
            if (gViewIdx[v] == gSelectedRow) { found = true; break; }
        }
        if (!found)
        {
            gSelectedRow = -1;
...
static void DrawCustomersTable(Rectangle area, bool allowInteraction)
{
    ...
    int startV = gScroll;
    int endV = startV + visibleRows;
    if (endV > gViewCount) endV = gViewCount;

    for (int vi = startV; vi < endV; vi++)
    {
        int i = gViewIdx[vi];
        float rowY = lineY + 8 + (vi - startV) * rowH;
        Rectangle rowR = { area.x + 2, rowY, area.width - 4, rowH - 4 };

        bool selected = (i == gSelectedRow);
        if (selected)
            DrawRectangleRec(rowR, (Color){230, 230, 230, 255});

        if (allowInteraction && CheckCollisionPointRec(GetMousePosition(), rowR))
        {
            DrawRectangleLines((int)rowR.x, (int)rowR.y, (int)rowR.width, (int)rowR.height, BLUE);
    ...
    if (gViewCount == 0)
    {
        DrawText("No customer data found.", (int)(area.x + 20), (int)(area.y + 70), 20, GRAY);
    }
}
...
void CashierCustomersPage(Rectangle contentArea, void *dbcPtr)
{
    ...
    DrawText("Search", (int)(filterArea.x + 10), (int)(filterArea.y + 12), 18, BLACK);
    ...
    // tombol selalu menempel bawah
    // btnY sudah dihitung di atas
