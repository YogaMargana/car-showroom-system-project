...
void AdminPenjualannobilPage(AppState *app, Rectangle contentArea, void *dbcPtr);
...
#define MAX_ROWS 200
static Penjualanmobildata gRows[MAX_ROWS];
...
static void ReloadRows(void *dbcPtr)
{
    ...
    int count = 0;
    if (DbPenjualanMobil_LoadAll(dbc, gRows, MAX_ROWS, &count))
    {
        gRowCount = count;
...
static void DrawTablePaged(Rectangle tableArea, bool enabled, void *dbcPtr)
{
    ...
    // kolom: ID Mobil Sales Kasir Pelanggan juml Tanggal Status Total Uang NoTrans Kembali Action
    // kolom baru: No | Tanggal | Mobil(NamaMobil) | Sales | Kasir | Pelanggan(Nama) | Qty | Total | Uang | Kembali | Status
// Dibuat lebih rapat supaya tabel enak dilihat, tapi tetap ada ruang (tidak mepet)
float colW[] = {
    44,  // No
    100, // Tanggal
    150, // Mobil
...
const char *hdr[] = {
    "No", "Tanggal", "Mobil", "Sales", "Kasir",
    "Pelanggan", "Qty", "Total", "Uang", "Kembali"
};
    ...
    for (int row = startRow; row < endRow; row++)
    {
        ...
        const Penjualanmobildata *p = &gRows[row];
     ...
     char noBuf[16];
snprintf(noBuf, sizeof(noBuf), "%d", row + 1); // nomor urut global

DrawText(noBuf, xx, textY, FONT_ROW, BLACK);
xx += colW[0];

DrawText(p->TanggalTransaksi, xx, textY, FONT_ROW, BLACK);
xx += colW[1];

// Mobil: tampilkan NamaMobil (bukan MobilID)
DrawText(p->NamaMobil, xx, textY, FONT_ROW, BLACK);
xx += colW[2];
...
// Pelanggan: tampilkan Nama pelanggan (bukan PelangganID)
DrawText(p->PelangganNama, xx, textY, FONT_ROW, BLACK);
xx += colW[5];
...
//         {
//             clickedAction = true;
//             strncpy(gPendingDeleteId, p->PenjualanMobilID, sizeof(gPendingDeleteId) - 1);
...
void AdminPenjualannobilPage(AppState *app, Rectangle contentArea, void *dbcPtr)
{
    ...
    DrawText("Penjualan Mobil", (int)contentArea.x + 40, (int)contentArea.y + 35, FONT_TITLE, RAYWHITE);
    ...
    if (!blocked)
    {
        ...
        {
            if (app)
                app->halamanSekarang = HAL_CASHIER_INPUT_PENJUALAN_MOBIL;
        }
    ...
    {
        UIModalResult r = UIDrawModalYesNo("Hapus data ini?", "Ya", "Tidak", 18);
        if (r == UI_MODAL_YES)
        {
            void *dbc = NULL;
            if (dbcPtr)
            ...
            {
                if (DbPenjualanMobil_Delete(dbc, gPendingDeleteId))
                {
                    gNeedReload = 1;
                    gSelected = -1;
                }
