...
// =======================================================
// Local helpers (page-level) SQL batch
// =======================================================
...
static long long GetAksesorisHargaLL(const Accessoris *a);
static long long CalcTotalCart(void);

static void ReloadAksesoris(void *dbcPtr);
...
static int FindCartIndexByIdx(int idx);
static int GetQtyByIdx(int idx);
static void CartSetQty(int idx, int qty);
static void CartInc(int idx);
static void CartDec(int idx);

static bool MiniButtonColored(Rectangle r, const char *label, Color base, Color hotCol);
static void DrawAksesorisTable(Rectangle area, bool allowInteraction);
...
// FIX BUG: kalau ID kosong tapi display ada, coba restore ID dari label
static void EnsureIdFromDisplay(const LookupItem *items, int itemCount,
                                const char *display, char *outId, int outIdSize);

// =======================================================
// DATA (KIRI)
// =======================================================
static Accessoris gAksesoris[MAX_ACCESSORIS];
static int gCount = 0;
...
typedef struct
{
    int idx; // index aksesoris di array
    int qty;
} CartItem;
...
// kasir lock (auto-fill jika login kasir)
...
// ✅ tambahan: pesan warning kalau uang kurang, popup tetap open
...
static void CloseAllPickers(void)
{
    ...
    // juga pastikan textbox tidak “nyangkut” active
...
static void DrawReadOnlyBox(Rectangle bounds, const char *text, int fontSize)
{
    DrawRectangleRec(bounds, (Color){235, 235, 235, 255});
...
static long long CalcTotalCart(void)
{
    ...
    for (int i = 0; i < gCartCount; i++)
    {
        ...
        if (qty <= 0)
            continue;
        long long harga = GetAksesorisHargaLL(&gAksesoris[idx]);
...
static void ReloadAksesoris(void *dbcPtr)
{
    ...
    int count = 0;
    if (DbAccesoriss_LoadAll(dbc, gAksesoris, MAX_ACCESSORIS, &count))
    {
        gCount = count;
        int maxPage = CeilDivInt(gCount, DEFAULT_PAGE_ROWS) - 1;
        if (maxPage < 0)
            maxPage = 0;
...
static void ReloadLookups(void *dbcPtr)
{
    ...
    if (DbCustomers_LoadAll(dbc, gCustRows, MAX_CUSTOMERS, &cc))
    {
        ...
        {
            snprintf(gPelanggans[gPelangganCount].id, sizeof(gPelanggans[gPelangganCount].id), "%s", gCustRows[i].PelangganID);
            snprintf(gPelanggans[gPelangganCount].label, sizeof(gPelanggans[gPelangganCount].label), "%s - %s",
                     gCustRows[i].PelangganID, gCustRows[i].Nama);
            gPelangganCount++;
        }
    }
    ...
    if (DbEmployees_LoadAll(dbc, gEmpRows, MAX_EMPLOYEES, &ec))
    {
        ...
        for (int i = 0; i < gEmpRowCount; i++)
        {
            if (strcmp(gEmpRows[i].Posisi, "Kasir") == 0)
            {
                ...
                {
                    snprintf(gKasirs[gKasirCount].id, sizeof(gKasirs[gKasirCount].id), "%s", gEmpRows[i].KaryawanID);
                    snprintf(gKasirs[gKasirCount].label, sizeof(gKasirs[gKasirCount].label), "%s - %s",
                             gEmpRows[i].KaryawanID, gEmpRows[i].Nama);
                    gKasirCount++;
                }
            }
            else if (strcmp(gEmpRows[i].Posisi, "Sales") == 0)
            {
                ...
                {
                    snprintf(gSalesList[gSalesCount].id, sizeof(gSalesList[gSalesCount].id), "%s", gEmpRows[i].KaryawanID);
                    snprintf(gSalesList[gSalesCount].label, sizeof(gSalesList[gSalesCount].label), "%s - %s",
                             gEmpRows[i].KaryawanID, gEmpRows[i].Nama);
                    gSalesCount++;
                }
            }
...
static void DrawSearchBox(Rectangle r, bool allowInput)
{
    ...
    DrawText(gSearch[0] ? gSearch : "cari nama aksesoris",
...
static void CartSetQty(int idx, int qty)
{
    ...
    {
        gCart[gCartCount].idx = idx;
        gCart[gCartCount].qty = qty;
        gCartCount++;
    }
}

static void CartInc(int idx) { CartSetQty(idx, GetQtyByIdx(idx) + 1); }
static void CartDec(int idx) { CartSetQty(idx, GetQtyByIdx(idx) - 1); }
...
static void DrawAksesorisTable(Rectangle area, bool allowInteraction)
{
    ...
    int rowsPerPage = DEFAULT_PAGE_ROWS;
    if (availH > rowH)
    {
    ...
    int filtered[MAX_ACCESSORIS];
    int fcnt = 0;

    for (int i = 0; i < gCount && fcnt < MAX_ACCESSORIS; i++)
    {
        if (!PassSearch(&gAksesoris[i]))
            continue;
        if (gAksesoris[i].Stok > 0)
            filtered[fcnt++] = i;
    }
    for (int i = 0; i < gCount && fcnt < MAX_ACCESSORIS; i++)
    {
    ...
    {
        const char *msg = "Data aksesoris tidak ketemu";
        int fsMsg = 18;
    ...
    for (int slot = 0; slot < rowsPerPage; slot++)
    {
        ...
        DrawTextClipped(nameClip, gAksesoris[i].NamaAksesoris, 16, BLACK, gScrollNameX);

        Rectangle stokClip = (Rectangle){colStock, row.y + 6, 60, 18};
        char stokStr[16];
        snprintf(stokStr, sizeof(stokStr), "%d", gAksesoris[i].Stok);
        DrawTextClipped(stokClip, stokStr, 16, BLACK, 0);

        char hargaRp[64];
        FormatRupiah(GetAksesorisHargaLL(&gAksesoris[i]), hargaRp, (int)sizeof(hargaRp));
        Rectangle hargaClip = (Rectangle){colHarga, row.y + 6, 160, 18};
        DrawTextClipped(hargaClip, hargaRp, 16, BLACK, 0);

        int qty = GetQtyByIdx(i);
        int stock = gAksesoris[i].Stok;
...
static void DrawHasilInputTableScroll(Rectangle area)
{
    ...
    for (int i = start; i < end; i++)
    {
        ...
        if (idx >= 0 && idx < gCount)
        {
            nama = gAksesoris[idx].NamaAksesoris;
            harga = GetAksesorisHargaLL(&gAksesoris[idx]);
        }
...
static void DrawRightForm(Rectangle rightPanel, void *dbcPtr, bool enabled)
{
    ...
    if (enabled)
    {
        // ✅ kasir locked -> jangan update textbox kasir, supaya ga bisa diketik
        if (!gLockKasir)
        ...
        // NOTE: sengaja TIDAK reset ID hanya karena textbox active,
        // karena ini yang sering bikin bug "udah select tapi jadi kosong".
        // ID akan kita "ensure" sebelum validasi Selanjutnya.
    }
...
#define APPEND_FMT(...)                                    \
    ...
    for (int i = 0; i < gCartCount; i++)
    {
        ...
        const Accessoris *a = &gAksesoris[idx];
        if (!a->AksesorisID[0])
            continue;
    ...
    APPEND_FMT("COMMIT; END TRY BEGIN CATCH IF @@TRANCOUNT > 0 ROLLBACK; THROW; END CATCH;");
...
// =======================================================
// FIX BUG: restore ID dari display (kalau ID kosong tapi display ada)
// =======================================================
static void EnsureIdFromDisplay(const LookupItem *items, int itemCount,
                                const char *display, char *outId, int outIdSize)
{
    ...
    // 1) cocokkan label persis
    for (int i = 0; i < itemCount; i++)
    {
        if (strcmp(items[i].label, display) == 0)
        {
            strncpy(outId, items[i].id, (size_t)outIdSize - 1);
            outId[outIdSize - 1] = '\0';
            return;
        }
    }

    // 2) parse prefix sebelum spasi / '-' (format label biasanya "ID - Nama")
    char id[32];
    int j = 0;
    for (const char *p = display; *p && j < (int)sizeof(id) - 1; p++)
    {
        if (*p == ' ' || *p == '-')
            break;
        id[j++] = *p;
    ...
    for (int i = 0; i < itemCount; i++)
    {
        ...
        {
            strncpy(outId, id, (size_t)outIdSize - 1);
            outId[outIdSize - 1] = '\0';
            return;
        }
    }
...
void AdminInputPenjualanaksesorisPage(AppState *app, Rectangle leftArea, Rectangle rightArea, void *dbcPtr)
{
    ...
    Rectangle tableAks = (Rectangle){leftPanel.x, leftPanel.y + 70, leftPanel.width, leftPanel.height - 70};
    DrawAksesorisTable(tableAks, !blocked);
    ...
    if (!blocked)
    {
        ...
        {
            // ✅ FIX BUG: pastikan ID keisi dari display sebelum validasi
            EnsureIdFromDisplay(gPelanggans, gPelangganCount, gPelangganDisplay, gPelangganID, (int)sizeof(gPelangganID));
            EnsureIdFromDisplay(gSalesList, gSalesCount, gSalesDisplay, gSalesID, (int)sizeof(gSalesID));
            if (!gLockKasir)
                EnsureIdFromDisplay(gKasirs, gKasirCount, gKasirDisplay, gKasirID, (int)sizeof(gKasirID));

            // pastikan focus textbox tidak nyangkut
            CloseAllPickers();
        ...
        // picker kasir hanya jika tidak lock
        if (!gLockKasir)
        {
    ...
    if (gPayPopupOpen)
    {
        ...
        if (ok)
        {
            // di sini sudah pasti uang cukup (karena OK tidak akan confirm jika kurang)
            void *dbc = NULL;
            if (dbcPtr)
                dbc = *(void **)dbcPtr;
