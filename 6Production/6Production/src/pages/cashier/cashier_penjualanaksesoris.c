...
static void DrawTablePaged(Rectangle tableArea, bool enabled, void *dbcPtr)
{
    ...
    // kolom: No | NoTrans | Tanggal | Aksesoris(nama) | Sales | Kasir | Pelanggan(nama) | Qty | Total | Uang | Kembali
    // dibuat lebih rapat, tapi tetap ada ruang supaya tidak "mepet".
    float colW[] = {
        44,  // No
        95,  // Tanggal
        155, // Aksesoris
    ...
    const char *hdr[] = {
        "No", "Tanggal", "Aksesoris", "Sales", "Kasir",
    ...
    for (int row = startRow; row < endRow; row++)
    {
        ...
        // No urut seperti halaman Car Sales
        char noBuf[16];
        snprintf(noBuf, sizeof(noBuf), "%d", row + 1);
        ...
        const char *aksTxt = (p->AksesorisNama[0] != '\0') ? p->AksesorisNama : p->AksesorisID;
        DrawText(aksTxt, xx, textY, FONT_ROW, BLACK);
        xx += colW[2];
        ...
        const char *pelTxt = (p->PelangganNama[0] != '\0') ? p->PelangganNama : p->PelangganID;
        DrawText(pelTxt, xx, textY, FONT_ROW, BLACK);
        xx += colW[5];
