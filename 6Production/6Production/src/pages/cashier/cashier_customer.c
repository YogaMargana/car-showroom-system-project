...
static int IsDuplicateCustomer(const char *noKtp, const char *email, const char *noHp, const char *excludeId)
{
    for (int i = 0; i < gCustomerCount; i++)
    {
        if (excludeId && excludeId[0] && strcmp(gCustomers[i].PelangganID, excludeId) == 0)
            continue;

        if (strcmp(gCustomers[i].NoKTP, noKtp) == 0)
            return 1;
        if (StringEqualsI(gCustomers[i].Email, email))
            return 1;
        if (strcmp(gCustomers[i].NoHp, noHp) == 0)
            return 1;
    }
    return 0;
}
...
{
    strncpy(gNama, c->Nama, sizeof(gNama) - 1);
    gNama[sizeof(gNama) - 1] = '\0';
    strncpy(gNoKtp, c->NoKTP, sizeof(gNoKtp) - 1);
    gNoKtp[sizeof(gNoKtp) - 1] = '\0';
    strncpy(gEmail, c->Email, sizeof(gEmail) - 1);
    gEmail[sizeof(gEmail) - 1] = '\0';
    strncpy(gNoHp, c->NoHp, sizeof(gNoHp) - 1);
    gNoHp[sizeof(gNoHp) - 1] = '\0';
    strncpy(gAlamat, c->Alamat, sizeof(gAlamat) - 1);
    gAlamat[sizeof(gAlamat) - 1] = '\0';
...
static void ReloadCustomers(void *dbcPtr)
{
    ...
    if (DbCustomers_LoadAll(dbc, gCustomers, MAX_CUSTOMERS, &count))
    {
        ...
        if (gSelectedRow >= gCustomerCount)
            gSelectedRow = -1;
    }
    else
    {
...
static void DrawCustomersTable(Rectangle area, bool allowInteraction)
{
    ...
    const float pad = 14.0f;    // jarak dari border tabel
    float colGap = 18.0f;       // jarak antar kolom
    const float textPad = 6.0f; // jarak teks dari awal kolom
    ...
    // ada 7 kolom: No | ID | Name | KTP | Email | Phone | Address
    // berarti ada 6 gap antar kolom
    float wAvail = w - (wNo + wId) - colGap * 6;

    // fallback kalau layar kecil: kecilkan gap biar tetap muat
    if (wAvail < 200)
    {
        colGap = 10.0f;
        wAvail = w - (wNo + wId) - colGap * 6;
        if (wAvail < 0)
            wAvail = 0;
    }

    // bagi lebar (alamat paling besar)
    float wName = wAvail * 0.16f;
    float wKtp = wAvail * 0.18f;
    float wEmail = wAvail * 0.18f;
    float wPhone = wAvail * 0.12f;
    float wAlamat = wAvail - (wName + wKtp + wEmail + wPhone);

    // posisi kolom (pakai + colGap)
    float colNo = x0;
    float colId = colNo + wNo + colGap;
    float colName = colId + wId + colGap;
    float colKTP = colName + wName + colGap;
    float colEmail = colKTP + wKtp + colGap;
    float colPhone = colEmail + wEmail + colGap;
    float colAlamat = colPhone + wPhone + colGap;

    DrawText("No", (int)(colNo + textPad), (int)headerY, 18, BLACK);
    DrawText("Customer ID", (int)(colId + textPad), (int)headerY, 18, BLACK);
    DrawText("Name", (int)(colName + textPad), (int)headerY, 18, BLACK);
    DrawText("No. KTP", (int)(colKTP + textPad), (int)headerY, 18, BLACK);
    DrawText("Email", (int)(colEmail + textPad), (int)headerY, 18, BLACK);
    DrawText("Phone", (int)(colPhone + textPad), (int)headerY, 18, BLACK);
    DrawText("Address", (int)(colAlamat + textPad), (int)headerY, 18, BLACK);
    ...
    for (int v = start; v < end; v++)
    {
        ...
        if (allowInteraction && hover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            if (gSelectedRow == i)
            {
                // klik row yang sama => unselect + clear
                gSelectedRow = -1;
                ClearForm();
            }
            else
            {
                // klik row lain => select + isi form
                gSelectedRow = i;
        ...
        int y = (int)rowY + 2;
        DrawText(noStr, (int)(colNo + textPad), y, 18, BLACK);
        DrawTextEllipsis(gCustomers[i].PelangganID, (int)(colId + textPad), y, 18, (int)(wId - textPad * 2), BLACK);
        DrawTextEllipsis(gCustomers[i].Nama, (int)(colName + textPad), y, 18, (int)(wName - textPad * 2), BLACK);
        DrawTextEllipsis(gCustomers[i].NoKTP, (int)(colKTP + textPad), y, 18, (int)(wKtp - textPad * 2), BLACK);
        DrawTextEllipsis(gCustomers[i].Email, (int)(colEmail + textPad), y, 18, (int)(wEmail - textPad * 2), BLACK);
        DrawTextEllipsis(gCustomers[i].NoHp, (int)(colPhone + textPad), y, 18, (int)(wPhone - textPad * 2), BLACK);
        DrawTextEllipsis(gCustomers[i].Alamat, (int)(colAlamat + textPad), y, 18, (int)(wAlamat - textPad * 2), BLACK);
...
void CashierCustomersPage(Rectangle contentArea, void *dbcPtr)
{
    void *dbc = NULL;
    if (dbcPtr)
        dbc = *(void **)dbcPtr;
    ...
    if (tableArea.height < 140)
        tableArea.height = 140;
    ...
    if (!blocked)
    {
        if (gSelectedRow < 0)
        {
            ...
            if (UIButton(btnRight, "+ Add", 18))
            {
                ...
                {
                    if (IsDuplicateCustomer(gNoKtp, gEmail, gNoHp, NULL))
                    ...
                    else if (DbCustomers_Insert(dbc, gNoKtp, gNama, gEmail, gNoHp, gAlamat))
        ...
        else
        {
            ...
            if (UIButton(btnLeft2, "Edit", 18))
            {
                ...
                {
                    if (IsDuplicateCustomer(gNoKtp, gEmail, gNoHp, gCustomers[gSelectedRow].PelangganID))
                    ...
                    else if (DbCustomers_Update(dbc, gCustomers[gSelectedRow].PelangganID, gNoKtp, gNama, gEmail, gNoHp, gAlamat))
    ...
    {
        UIModalResult r = UIDrawModalOK("Data tidak boleh sama !", "OK", 18);
        if (r == UI_MODAL_OK)
            gModal = MODAL_NONE;
    }
        ...
        else if (r == UI_MODAL_YES)
        {
            if (dbc && gPendingDeleteId[0] != '\0')
            {
                if (DbCustomers_Delete(dbc, gPendingDeleteId))
                {
                    ...
                    ClearForm();
                    gNeedReload = 1;
                    SetToast("Customer Deleted (Deactivated)");
                }
                else
                {
