...
/* Display buffers (show name in textbox) */
static char gMobilDisplay[64] = "";
static char gKaryawanDisplay[64] = "";
...
static const char *kStatuses[] = {"Dijadwalkan", "Dibatalkan", "Selesai"};
...
/* Date picker (calendar) for tbTanggal */
static UIDatePicker dpTanggal;

static char gTableSearch[64] = "";

/* Status filter dropdown (table) */
static const char *kStatusFilterItems[] = {"All", "Dijadwalkan", "Dibatalkan", "Selesai"};
...
/* Picker scroll offsets (so dropdown list can be scrolled) */
static int gPickMobilScroll = 0;
static int gPickKaryawanScroll = 0;
...
static const char *FindLabelById(const LookupItem *items, int count, const char *id)
{
    if (!items || count <= 0 || !id || id[0] == '\0')
        return NULL;

    for (int i = 0; i < count; i++)
    {
        if (IdEqualsTrim(id, items[i].id))
            return items[i].label;
    }
    return NULL;
}

static void ExtractNameOnly(const char *label, char *out, int outSize)
{
    if (!out || outSize <= 0)
        return;
    ...
    const char *name = sep ? (sep + 3) : label;

    strncpy(out, name, (size_t)outSize - 1);
    out[outSize - 1] = '\0';
}

static void SetDisplayFromId(const LookupItem *items, int count, const char *id, char *out, int outSize)
{
    const char *label = FindLabelById(items, count, id);
    if (label)
    {
        ExtractNameOnly(label, out, outSize);
    }
    else
    {
        if (!out || outSize <= 0)
        ...
        strncpy(out, id ? id : "", (size_t)outSize - 1);
        out[outSize - 1] = '\0';
    }
}

/* Draw text truncated with "..." to fit within maxW pixels */
static void DrawTextFit(const char *txt, int x, int y, int fontSize, Color color, int maxW)
{
    if (!txt)
        txt = "";

    if (maxW <= 0 || MeasureText(txt, fontSize) <= maxW)
    {
        DrawText(txt, x, y, fontSize, color);
        return;
    }
    ...
    const char *ellipsis = "...";
    int ellW = MeasureText(ellipsis, fontSize);

    int len = (int)strlen(buf);
    while (len > 0)
    {
        buf[len] = '\0';
        if (MeasureText(buf, fontSize) + ellW <= maxW)
            break;
        len--;
    }
    ...
    if (len <= 0)
    {
        DrawText(ellipsis, x, y, fontSize, color);
        return;
    }

    strncat(buf, ellipsis, sizeof(buf) - strlen(buf) - 1);
...
static void CloseAllPickers(void)
{
    ...
    /* reset scroll offsets so picker list starts from top next time */
    gPickMobilScroll = 0;
    gPickKaryawanScroll = 0;
    gPickPelangganScroll = 0;
}

static void ClearForm(void)
{
    gMobilID[0] = gKaryawanID[0] = gPelangganID[0] = gTanggal[0] = '\0';
    gMobilDisplay[0] = gKaryawanDisplay[0] = gPelangganDisplay[0] = '\0';
    gOrigKaryawanID[0] = gOrigTanggal[0] = '\0';
    tbMobil.len = tbKaryawan.len = tbPelanggan.len = tbTanggal.len = 0;
    gStatusIndex = 0;
    dpTanggal.open = false;
    CloseAllPickers();
}
...
{
    strncpy(gMobilID, td->MobilID, sizeof(gMobilID) - 1);
    gMobilID[sizeof(gMobilID) - 1] = '\0';
    strncpy(gKaryawanID, td->KaryawanID, sizeof(gKaryawanID) - 1);
    gKaryawanID[sizeof(gKaryawanID) - 1] = '\0';
    strncpy(gPelangganID, td->PelangganID, sizeof(gPelangganID) - 1);
    gPelangganID[sizeof(gPelangganID) - 1] = '\0';
    strncpy(gTanggal, td->Tanggal, sizeof(gTanggal) - 1);
    gTanggal[sizeof(gTanggal) - 1] = '\0';
    ...
    SetDisplayFromId(gMobils, gMobilCount, gMobilID, gMobilDisplay, (int)sizeof(gMobilDisplay));
    SetDisplayFromId(gSalesEmployees, gSalesCount, gKaryawanID, gKaryawanDisplay, (int)sizeof(gKaryawanDisplay));
    SetDisplayFromId(gCustomers, gCustomerCount, gPelangganID, gPelangganDisplay, (int)sizeof(gPelangganDisplay));

    tbMobil.len = (int)strlen(gMobilDisplay);
    tbKaryawan.len = (int)strlen(gKaryawanDisplay);
    tbPelanggan.len = (int)strlen(gPelangganDisplay);
...
static int ParseDateYMD(const char *s, int *y, int *m, int *d)
{
    ...
    if (!chk)
        return 0;
    if (chk->tm_year != tmv.tm_year || chk->tm_mon != tmv.tm_mon || chk->tm_mday != tmv.tm_mday)
        return 0;
...
static void BuildView(void)
{
    ...
    {
        if (statusFilter && strcmp(gTd[i].Status, statusFilter) != 0)
            continue;

        if (q && q[0])
        {
            const char *mobLabel = FindLabelById(gMobils, gMobilCount, gTd[i].MobilID);
            const char *salesLabel = FindLabelById(gSalesEmployees, gSalesCount, gTd[i].KaryawanID);
            const char *custLabel = FindLabelById(gCustomers, gCustomerCount, gTd[i].PelangganID);
            ...
            if (!StrContainsI(gTd[i].TestDriveID, q) &&
                ...
                !(mobLabel && StrContainsI(mobLabel, q)) &&
                !(salesLabel && StrContainsI(salesLabel, q)) &&
                !(custLabel && StrContainsI(custLabel, q)))
            {
                continue;
            }
        }

        if (gViewCount < MAX_VIEW)
...
{
    if (gToastTimer <= 0.0f || gToast[0] == '\0')
        return;
                       ...
                       float maxWidth)
{
    ...
    if (maxWidth > dropW)
        dropW = maxWidth;
    float maxDropH = 260.0f;
    int visibleRows = (int)((maxDropH - 10.0f) / rowH);
    if (visibleRows < 1)
        visibleRows = 1;
    int showRows = (mc <= 0) ? 1 : (mc < visibleRows ? mc : visibleRows);
    float dropH = (float)showRows * rowH + 10.0f;

    Rectangle drop = {tbBounds.x, tbBounds.y + tbBounds.height + 6.0f, dropW, dropH};

    // keep inside screen; if hitting bottom, open upward
    int sw = GetScreenWidth();
    int sh = GetScreenHeight();
    if (drop.x + drop.width > sw - 10)
        drop.x = (float)(sw - (int)drop.width - 10);
    if (drop.x < 10)
        drop.x = 10;
    if (drop.y + drop.height > sh - 10)
        drop.y = tbBounds.y - drop.height - 6.0f;
    if (drop.y < 10)
        drop.y = 10;
    ...
    // mouse wheel scroll (only when hover dropdown)
    if (scroll && mc > visibleRows && CheckCollisionPointRec(m, drop))
    {
    ...
    int maxScroll = mc - visibleRows;
    if (maxScroll < 0)
        maxScroll = 0;
    int start = scroll ? *scroll : 0;
    start = ClampInt(start, 0, maxScroll);
    if (scroll)
        *scroll = start;
    ...
    for (int r = start; r < end; r++)
    {
        int idx = matches[r];
        int drawRow = r - start;
        Rectangle row = {drop.x + 5, drop.y + 5 + (float)drawRow * rowH, drop.width - 16, rowH};

        bool hover = CheckCollisionPointRec(m, row);
        if (hover)
            DrawRectangleRec(row, (Color){220, 230, 255, 255});
        ...
        if (hover && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            ...
            /* Show the NAME in textbox, keep the ID for database */
            ExtractNameOnly(items[idx].label, displayBuf, displayBufSize);
            tb->len = (int)strlen(displayBuf);
    ...
    if (mc > visibleRows)
    {
        Rectangle track = {drop.x + drop.width - 10, drop.y + 5, 6, drop.height - 10};
        DrawRectangleRec(track, (Color){230, 230, 230, 255});
        DrawRectangleLines((int)track.x, (int)track.y, (int)track.width, (int)track.height, (Color){160, 160, 160, 255});

        float trackH = track.height;
        float knobH = trackH * ((float)visibleRows / (float)mc);
        if (knobH < 18.0f)
            knobH = 18.0f;
        if (knobH > trackH)
            knobH = trackH;

        float t = (maxScroll > 0) ? ((float)start / (float)maxScroll) : 0.0f;
        float knobY = track.y + t * (trackH - knobH);
        Rectangle knob = {track.x + 1, knobY + 1, track.width - 2, knobH - 2};
        DrawRectangleRec(knob, (Color){170, 170, 170, 255});
    }
}
...
static int ValidateForm(int requireFutureDate, int enforceSalesOnly)
{
    ...
    if (!IsIdInList(gMobilID, gMobils, gMobilCount))
        return 0;
    if (!IsIdInList(gPelangganID, gCustomers, gCustomerCount))
        return 0;
    /* Sales ID: always required; for legacy rows we only enforce Sales-role when the field is changed */
    if (!gKaryawanID[0])
        return 0;
    if (enforceSalesOnly && !IsIdInList(gKaryawanID, gSalesEmployees, gSalesCount))
        return 0;

    /* Date must be valid YYYY-MM-DD; for insert we require today/future */
    if (!IsDateValid(gTanggal))
        return 0;
    if (requireFutureDate && !IsDateTodayOrFuture(gTanggal))
        return 0;

    if (gStatusIndex < 0 || gStatusIndex >= kStatusCount)
        return 0;
    return 1;
}
...
static void DrawTestDriveTable(Rectangle area, bool allowInteraction)
{
    ...
    DrawText("Mobil", (int)colMob, (int)headerY, 18, BLACK);
    DrawText("Sales", (int)colKar, (int)headerY, 18, BLACK);
    DrawText("Customer", (int)colPel, (int)headerY, 18, BLACK);
    ...
    for (int vi = start; vi < end; vi++)
    {
        ...
        SetDisplayFromId(gMobils, gMobilCount, gTd[i].MobilID, mobName, (int)sizeof(mobName));
        SetDisplayFromId(gSalesEmployees, gSalesCount, gTd[i].KaryawanID, salesName, (int)sizeof(salesName));
        SetDisplayFromId(gCustomers, gCustomerCount, gTd[i].PelangganID, custName, (int)sizeof(custName));

        int wMob = (int)(colKar - colMob - 10);
        int wSales = (int)(colPel - colKar - 10);
        int wCust = (int)(colTgl - colPel - 10);

        DrawTextFit(mobName[0] ? mobName : gTd[i].MobilID, (int)colMob, (int)row.y + 4, 16, BLACK, wMob);
        DrawTextFit(salesName[0] ? salesName : gTd[i].KaryawanID, (int)colKar, (int)row.y + 4, 16, BLACK, wSales);
        DrawTextFit(custName[0] ? custName : gTd[i].PelangganID, (int)colPel, (int)row.y + 4, 16, BLACK, wCust);
...
static void DrawStatusSelector(Rectangle bounds, int enabled)
{
    ...
    if (enabled)
    {
        if (UIButton(left, "<", 18))
        {
            gStatusIndex--;
            if (gStatusIndex < 0)
                gStatusIndex = kStatusCount - 1;
        }
        if (UIButton(right, ">", 18))
        {
            gStatusIndex++;
            if (gStatusIndex >= kStatusCount)
                gStatusIndex = 0;
        }
    }
    else
                             ...
                             bool *consumedClick, int overlayOnly)
{
    ...
    if (!overlayOnly)
    {
        ...
        if (*selectedIndex >= 0 && *selectedIndex < kStatusFilterCount)
            label = kStatusFilterItems[*selectedIndex];
...
void SalesTestDrivePage(Rectangle contentArea, void *dbcPtr)
{
    ...
    if (!gUiInited)
    {
        UITextBoxInit(&tbMobil, (Rectangle){0, 0, 0, 0}, gMobilDisplay, (int)sizeof(gMobilDisplay), false);
        UITextBoxInit(&tbKaryawan, (Rectangle){0, 0, 0, 0}, gKaryawanDisplay, (int)sizeof(gKaryawanDisplay), false);
        ...
        UIDatePickerInit(&dpTanggal, 0, 0);
        gUiInited = 1;
    }
    ...
    float col1X = labelX + 190;
    float col2LabelX = formArea.x + (formArea.width / 2) + 100;
    float col2X = col2LabelX + 160;

    float rowY = formArea.y + 18;

    DrawText("Mobil", (int)labelX, (int)rowY + 8, 18, BLACK);
    tbMobil.bounds = (Rectangle){col1X, rowY, 200, 34};

    DrawText("Customer", (int)col2LabelX, (int)rowY + 8, 18, BLACK);
    tbPelanggan.bounds = (Rectangle){col2X, rowY, 200, 34};

    rowY += 52;

    /* ===== SWAPPED AGAIN: LEFT = Status, RIGHT = Sales ID ===== */
    ...
    DrawText("Sales", (int)col2LabelX, (int)rowY + 8, 18, BLACK);
    tbKaryawan.bounds = (Rectangle){col2X, rowY, 200, 34};
    /* ========================================================== */

    rowY += 52;

    /* ===== ROW BELOW: Date / Jadwal (Calendar) ===== */
    DrawText("Date (YYYY-MM-DD)", (int)labelX, (int)rowY + 8, 18, BLACK);
    tbTanggal.bounds = (Rectangle){col1X, rowY, 200, 34};
    DrawText("Click to open calendar", (int)(col1X + 255), (int)rowY + 10, 16, BLACK);

    /* Textbox updates */
    if (allowInteraction)
    {
        /* If user types manually, clear the stored ID so they must pick from the dropdown again */
        char beforeMobil[64];
        char beforePelanggan[64];
        char beforeKaryawan[64];

        strncpy(beforeMobil, gMobilDisplay, sizeof(beforeMobil) - 1);
        beforeMobil[sizeof(beforeMobil) - 1] = '\0';
        strncpy(beforePelanggan, gPelangganDisplay, sizeof(beforePelanggan) - 1);
        beforePelanggan[sizeof(beforePelanggan) - 1] = '\0';
        strncpy(beforeKaryawan, gKaryawanDisplay, sizeof(beforeKaryawan) - 1);
        beforeKaryawan[sizeof(beforeKaryawan) - 1] = '\0';

        UITextBoxUpdate(&tbMobil);
        if (strcmp(beforeMobil, gMobilDisplay) != 0)
            gMobilID[0] = '\0';

        UITextBoxUpdate(&tbPelanggan);
        if (strcmp(beforePelanggan, gPelangganDisplay) != 0)
            gPelangganID[0] = '\0';

        UITextBoxUpdate(&tbKaryawan);
        if (strcmp(beforeKaryawan, gKaryawanDisplay) != 0)
            gKaryawanID[0] = '\0';
    ...
    /* Open calendar when clicking the date textbox */
    if (allowInteraction && IsMouseButtonPressed(MOUSE_LEFT_BUTTON) && CheckCollisionPointRec(GetMousePosition(), tbTanggal.bounds))
    {
        CloseAllPickers();
        dpTanggal.open = true;
    }
    ...
    if (allowInteraction)
    {
        DrawPicker(tbMobil.bounds, &tbMobil, gMobilID, (int)sizeof(gMobilID), gMobilDisplay, (int)sizeof(gMobilDisplay), gMobils, gMobilCount, &gPickMobilOpen, &gPickMobilScroll, 360.0f);
        DrawPicker(tbPelanggan.bounds, &tbPelanggan, gPelangganID, (int)sizeof(gPelangganID), gPelangganDisplay, (int)sizeof(gPelangganDisplay), gCustomers, gCustomerCount, &gPickPelangganOpen, &gPickPelangganScroll, 360.0f);
        DrawPicker(tbKaryawan.bounds, &tbKaryawan, gKaryawanID, (int)sizeof(gKaryawanID), gKaryawanDisplay, (int)sizeof(gKaryawanDisplay), gSalesEmployees, gSalesCount, &gPickKaryawanOpen, &gPickKaryawanScroll, 360.0f);
    }
    ...
    /* ===== Calendar overlay (draw after content so it isn't covered) ===== */
    if (UIDatePickerUpdateDraw(&dpTanggal, tbTanggal.bounds, gTanggal, (int)sizeof(gTanggal)))
    {
        tbTanggal.len = (int)strlen(gTanggal);
    }
