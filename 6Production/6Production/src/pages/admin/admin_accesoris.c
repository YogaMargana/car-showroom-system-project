/* --- START OF FILE admin_accesoris.c --- */
...
static char gPendingDeleteId[8] = ""; 
...
static int IsDuplicateAccessory(const char *nama, const char *merk, const char *excludeId)
{
    for (int i = 0; i < gEmpCount; i++) {
        if (excludeId && excludeId[0] && strcmp(gEmp[i].AksesorisID, excludeId) == 0) continue;
        if (StringEqualsI(gEmp[i].NamaAksesoris, nama) && StringEqualsI(gEmp[i].MerkAksesoris, merk)) {
            return 1;
        }
    }
    return 0;
...
/* --- Helper: Hapus karakter non-digit (untuk parsing ke DB) --- */
...
{
    // Menggunakan logika yang sama dengan StripFormat tapi langsung ke buffer sendiri
    // Ini aman karena hasil string pasti lebih pendek atau sama panjang
    char *src = buffer;
    char *dst = buffer;
    while (*src) {
...
/* --- Helper: Format angka dengan titik ribuan (misal: 1000 -> 1.000) --- */
static void AutoFormatPrice(char *buffer, int maxLen)
{
    char raw[64];
    StripFormat(raw, buffer); // Ambil angka murninya saja
    ...
    // Hitung panjang hasil akhir dengan titik
    int numDots = (len - 1) / 3;
    int newLen = len + numDots;

    if (newLen >= maxLen) return; // Buffer overflow protection
...
static int ValidateAccessoryForm(void)
{
    ...
    // Validasi Stok
    if (strlen(gStok) == 0) return 0;
    int stokVal = ParseIntSafe(gStok); // ParseIntSafe sudah handle digits only
    if (stokVal < 0) return 0;

    // Validasi Harga 
    if (strlen(gHarga) == 0) return 0;
...
static void CopyToForm(const Accessoris *e)
{
    ...
    // Copy Harga dan langsung format dengan titik
    snprintf(gHarga, sizeof(gHarga), "%d", e->Harga);
    AutoFormatPrice(gHarga, sizeof(gHarga));
...
void AdminAccessorissPage(Rectangle contentArea, void *dbcPtr)
{
    ...
    /* Layout untuk Rp. dan Input Harga */
    float rpWidth = 35.0f;
    DrawText("Rp.", (int)inputX, (int)rowY + 8, 18, BLACK);
    tbHarga.bounds = (Rectangle){inputX + rpWidth, rowY, boxW2 - rpWidth, boxH};

    if (!blocked) {
        UITextBoxUpdate(&tbNamaAksesoris);
        UITextBoxUpdate(&tbMerkAksesoris);
        
        // --- LOGIKA STOK HANYA ANGKA ---
        UITextBoxUpdate(&tbStok);
        SanitizeDigits(gStok); // Paksa hapus huruf
        tbStok.len = (int)strlen(gStok); // Update panjang cursor text

        // --- LOGIKA HARGA DENGAN TITIK ---
        ...
        if (gSelected < 0)
        {
            ...
            if (UIButton(btnRight, "+ Add", 18))
            {
                ...
                else if (dbc)
                {
                    ...
                    else if (IsDuplicateAccessory(gNamaAksesoris, gMerkAksesoris, NULL))
                    ...
                    else if (DbAccesoriss_Insert(dbc, gNamaAksesoris, gMerkAksesoris, stokVal, hargaVal))
        ...
        else
        {
            ...
            if (UIButton(btnLeft2, "Edit", 18))
            {
                ...
                else if (dbc && gSelected >= 0 && gSelected < gEmpCount)
                {
                    ...
                    else if (IsDuplicateAccessory(gNamaAksesoris, gMerkAksesoris, gEmp[gSelected].AksesorisID))
                    ...
                    else if (DbAccesoriss_Update(dbc, gEmp[gSelected].AksesorisID, gNamaAksesoris, gMerkAksesoris, stokVal, hargaVal))
        ...
        if (r == UI_MODAL_NO) {
            ...
            if (dbc && gPendingDeleteId[0] != '\0') {
                if (DbAccesoriss_Delete(dbc, gPendingDeleteId)) {
                    ...
                    ClearForm();
                    gNeedReload = 1;
                    SetToast("Aksesoris Deleted (Deactivated)");
                } else {
