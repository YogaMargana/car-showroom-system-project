                                        ...
                                        SalesTopSalesRow *out, int cap)
{
    ...
    // Panggil SP: usp_SalesReport_TopSales @From, @To, @TopN
    // @TopN kita set = cap (atau 5/6 sesuai UI)
    ...
    int n = 0;
    while (n < cap && SQLFetch(stmt) == SQL_SUCCESS) {
...
static const char *Q_SUMMARY =
    ...
    "   WHERE pm.StatusPembayaran IN ('Success','Succeed')) AS TotalMobilTerjual, "

    "  (SELECT ISNULL(SUM(pad.Qty),0) "
    "   FROM dbo.PenjualanAksesoris pa "
    "   JOIN dbo.PenjualanAksesorisDetail pad ON pad.PenjualanAksesorisID = pa.PenjualanAksesorisID "
    "   WHERE pa.StatusPembayaran IN ('Success','Succeed')) AS TotalAksesorisTerjual, "

    "  (SELECT ISNULL(SUM(CAST(pm.Total AS money)),0) "
    "   FROM dbo.PenjualanMobil pm "
    "   WHERE pm.StatusPembayaran IN ('Success','Succeed')) AS OmzetMobil, "

    "  (SELECT ISNULL(SUM(CAST(pad.Subtotal AS money)),0) "
    "   FROM dbo.PenjualanAksesoris pa "
    "   JOIN dbo.PenjualanAksesorisDetail pad ON pad.PenjualanAksesorisID = pa.PenjualanAksesorisID "
    "   WHERE pa.StatusPembayaran IN ('Success','Succeed')) AS OmzetAksesoris, "

    "  ( "
    "    (SELECT COUNT(*) FROM dbo.PenjualanMobil pm WHERE pm.StatusPembayaran IN ('Success','Succeed')) "
    "    + "
    "    (SELECT COUNT(*) FROM dbo.PenjualanAksesoris pa WHERE pa.StatusPembayaran IN ('Success','Succeed')) "
    "  ) AS SuccessTx;";

static const char *Q_MONTHLY =
    ";WITH MonthlyCar AS ( "
    "  SELECT CONVERT(char(7), pm.TanggalTransaksi, 120) AS Bulan, "
    "         COUNT(*) AS TxnCar, "
    "         SUM(CAST(pm.Total AS money)) AS OmzetCar "
    "  FROM dbo.PenjualanMobil pm "
    "  WHERE pm.StatusPembayaran IN ('Success','Succeed') "
    "  GROUP BY CONVERT(char(7), pm.TanggalTransaksi, 120) "
    "), "
    "MonthlyAcc AS ( "
    "  SELECT CONVERT(char(7), pa.TanggalTransaksi, 120) AS Bulan, "
    "         COUNT(DISTINCT pa.PenjualanAksesorisID) AS TxnAcc, "
    "         SUM(CAST(pad.Subtotal AS money)) AS OmzetAcc "
    "  FROM dbo.PenjualanAksesoris pa "
    "  JOIN dbo.PenjualanAksesorisDetail pad ON pad.PenjualanAksesorisID = pa.PenjualanAksesorisID "
    "  WHERE pa.StatusPembayaran IN ('Success','Succeed') "
    "  GROUP BY CONVERT(char(7), pa.TanggalTransaksi, 120) "
    "), "
    "Months AS ( "
...
static const char *Q_TOP_TX =
    ";WITH EmpCar AS ( "
    "  SELECT k.Nama AS Employee, COUNT(*) AS TxnCount "
    "  FROM dbo.PenjualanMobil pm "
    ...
    "), "
    "EmpAcc AS ( "
    "  SELECT k.Nama AS Employee, COUNT(DISTINCT pa.PenjualanAksesorisID) AS TxnCount "
    "  FROM dbo.PenjualanAksesoris pa "
    "  JOIN dbo.Karyawan k ON k.KaryawanID = pa.KaryawanID "
    "  WHERE pa.StatusPembayaran IN ('Success','Succeed') "
    "  GROUP BY k.Nama "
    "), "
    "EmpAll AS ( "
...
static const char *Q_TOP_REVENUE =
    ";WITH EmpCar AS ( "
    "  SELECT k.Nama AS Employee, SUM(CAST(pm.Total AS money)) AS TotalSales "
    "  FROM dbo.PenjualanMobil pm "
    "  JOIN dbo.Karyawan k ON k.KaryawanID = pm.KaryawanID "
    "  WHERE pm.StatusPembayaran IN ('Success','Succeed') "
    "  GROUP BY k.Nama "
    "), "
    "EmpAcc AS ( "
    "  SELECT k.Nama AS Employee, SUM(CAST(pad.Subtotal AS money)) AS TotalSales "
    "  FROM dbo.PenjualanAksesoris pa "
    "  JOIN dbo.PenjualanAksesorisDetail pad ON pad.PenjualanAksesorisID = pa.PenjualanAksesorisID "
    "  JOIN dbo.Karyawan k ON k.KaryawanID = pa.KaryawanID "
    "  WHERE pa.StatusPembayaran IN ('Success','Succeed') "
...
static const char *Q_BEST_CAR =
    ...
    "  COUNT(*) AS UnitTerjual, "
    "  SUM(CAST(pm.Total AS money)) AS TotalPenjualan "
    "FROM dbo.PenjualanMobil pm "
