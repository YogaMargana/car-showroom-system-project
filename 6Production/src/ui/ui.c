...
{
    Vector2 sz = MeasureTextEx(UIGetFont(), text, (float)fontSize, UIGetFontSpacing());
    return (int)sz.x;
}

void UIDrawText(const char *text, int x, int y, int fontSize, Color color)
{
    DrawTextEx(UIGetFont(), text, (Vector2){(float)x, (float)y}, (float)fontSize, UIGetFontSpacing(), color);
}
