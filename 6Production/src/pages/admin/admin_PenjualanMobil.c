...
static void DrawTablePaged(Rectangle tableArea, bool enabled, void *dbcPtr)
{
    ...
    // kolom: ID Mobil Sales Kasir Pelanggan Qty Tanggal Status Total Uang NoTrans Kembali Action
    float colW[] = {
        70,  // ID
        120, // NoTrans
        110, // Tanggal
    ...
    const char *hdr[] = {
        "ID", "NoTrans", "Tanggal", "Mobil", "Sales", "Kasir",
        "Pelanggan", "Qty", "Total", "Uang", "Kembali", "Status", "Action"};
    float totalW = 0;
    ...
    for (int row = startRow; row < endRow; row++)
    {
        ...
        xx += colW[6];
        DrawText(p->Qty, xx, textY, FONT_ROW, BLACK);
        xx += colW[7];
        ...
        Rectangle btnDel = {actionX + 86, rowY + 6, 70, 20};
        ...
        if (!clickedAction && CheckCollisionPointRec(mouse, rowRect) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
