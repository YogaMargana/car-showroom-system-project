...
static void DrawRoleSelector(Rectangle bounds, bool enabled)
{
    ...
    {
        if (gRoleIndex < 0) gRoleIndex = 0;
        else gRoleIndex = (gRoleIndex + 3 - 1) % 3;
        SyncPosisiFromRoleIndex();
    }
    if (ArrowButton(right, ">", enabled))
    {
        if (gRoleIndex < 0) gRoleIndex = 0;
        else gRoleIndex = (gRoleIndex + 1) % 3;
        SyncPosisiFromRoleIndex();
    }
...
{
    strncpy(gNama, e->Nama, sizeof(gNama)-1); gNama[sizeof(gNama)-1] = '\0';
    strncpy(gPosisi, e->Posisi, sizeof(gPosisi)-1); gPosisi[sizeof(gPosisi)-1] = '\0';
    strncpy(gNoHP, e->NoHP, sizeof(gNoHP)-1); gNoHP[sizeof(gNoHP)-1] = '\0';
    strncpy(gEmail, e->Email, sizeof(gEmail)-1); gEmail[sizeof(gEmail)-1] = '\0';
    strncpy(gUsername, e->Username, sizeof(gUsername)-1); gUsername[sizeof(gUsername)-1] = '\0';
    strncpy(gPassword, e->Password, sizeof(gPassword)-1); gPassword[sizeof(gPassword)-1] = '\0';
...
{
    if (!strlen(gNama)) return 0;
    if (gRoleIndex < 0 || gRoleIndex >= 3) return 0;
    if (!strlen(gNoHP) || !IsDigitsOnly(gNoHP)) return 0;
    int lenHp = (int)strlen(gNoHP);
    if (lenHp < 10 || lenHp > 15) return 0;

    if (!strlen(gEmail) || !IsValidEmail(gEmail)) return 0;
    if (!strlen(gUsername)) return 0;
    if (!strlen(gPassword)) return 0;
...
static void ReloadEmployees(void *dbcPtr)
{
    ...
    if (DbEmployees_LoadAll(dbc, gEmp, MAX_EMP, &count)) {
        ...
        if (gSelected >= gEmpCount) gSelected = -1;
    } else {
        gEmpCount = 0;
        gScroll = 0;
        gSelected = -1;
...
static void BuildEmployeesView(void)
{
    ...
    int visibleGuess = 8;
    int maxScroll = gViewCount - visibleGuess;
    if (maxScroll < 0) maxScroll = 0;
    gScroll = ClampInt(gScroll, 0, maxScroll);
}
...
static void DrawEmployeesTable(Rectangle area, bool allowInteraction)
{
    ...
    float colPos = area.x + pad + 420;
    float colPhone = area.x + pad + 520;
    ...
    if (allowInteraction)
    {
        ...
        if (CheckCollisionPointRec(mouse, area))
        {
            ...
            if (mw != 0.0f)
                gScroll = ClampInt(gScroll - (int)mw, 0, maxScroll);
        }
    }
...
void AdminEmployeesPage(Rectangle contentArea, void *dbcPtr)
{
    ...
    // 2) Position (dropdown) Fungsinya ada dibawah tinggal scroll aja, soalnya biar gak ketiban textbox
    rowY += rowH;
        ...
        if (gSelected < 0) {
            ...
            if (UIButton(btnRight, "+ Add", 18)) {
                ...
                if (!ValidateEmployeeForm()) {
                    ...
                    if (DbEmployees_Insert(dbc, gNama, gPosisi, gNoHP, gEmail, gUsername, gPassword)) {
                        ClearForm();
                        gNeedReload = 1;
            ...
            if (UIButton(btnLeft2, "Edit", 18)) {
                ...
                if (!ValidateEmployeeForm()) {
                    ...
                    if (DbEmployees_Update(dbc, gEmp[gSelected].KaryawanID, gNama, gPosisi, gNoHP, gEmail, gUsername, gPassword)) {
                        gNeedReload = 1;
